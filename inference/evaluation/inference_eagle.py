...
from spec_bench.eval import run_eval
...
def eagle_forward(inputs, model, tokenizer, max_new_tokens, max_length, teminators):
    ...
    # generate
    output_ids, accept_length_list, model_step, generate_time = model.generate(
        input_ids=input_ids,
    ...
    return output_ids, new_token, model_step, accept_length_list, generate_time
...
if __name__ == "__main__":
    ...
    model.init_storage()
    if args.V == 73440:
        token_id_remap = torch.arange(args.V, dtype=torch.int32, device="cpu")
        model._load("token_id_remap", token_id_remap, cls="eagle")
    else:
        import pickle
        with open(f"{args.eagle_path}/freq_{args.V}.pkl", "rb") as f:
            freq = pickle.load(f)
