...
def _save_artifacts(
    ...
    success_path = os.path.join(export_model_dir, base_file_name + ".success")
    ...
    if bmt.rank() == 0:
        ...
        with open(success_path, "w") as fpout:
            pass
...
def load_model_ckpt(args, model):
    ...
    bmt.load(model, checkpoint_file, strict=True)
    bmt.print_rank("model loading is done!!!")
    return model
...
def merge_dataset_ckpts(load_dir, parallel_load_datastate):
    ...
    def load_and_aggregate(p_dataset):
        ...
        def load_state_dict_with_npy(p_dataset):
            ...
            for key in state_dict:
                if isinstance(state_dict[key]["used"], str):
                    state_dict[key]["used"] = BitSet.load(
                        os.path.join(load_dir, "dataset_ckpt", os.path.basename(state_dict[key]["used"]))
                    )
            return state_dict
    ...
    if p_datasets:
        ...
        with ThreadPoolExecutor(
            max_workers=parallel_load_datastate
        ) as executor:  # smaller than normal load to avoid OOM
